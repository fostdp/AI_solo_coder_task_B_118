...
mod tests {
    ...
    #[test]
    fn test_slag_analysis() {
        let system = SlagAnalysisSystem::new();

        let request = SlagAnalysisRequest {
            composition: SlagComposition::default(),
            furnace_type: Some(crate::models::FurnaceType::HanChaogang),
        };
        ...
        assert!(result.process_inference.confidence > 0.0);
        assert!(!result.ore_source_candidates.is_empty());
    }

    #[test]
    fn test_slag_basicity() {
        let composition = SlagComposition {
            sio2: 0.4,
            cao: 0.4,
